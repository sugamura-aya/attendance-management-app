...
@section('css')
  {{-- 外部CSSを読み込む --}}
  <link rel="stylesheet" href="{{ asset('css/admin/showStaffAttendance.css') }}">
@endsection
    ...
    <div class="staff-attendance-list__content">
        ...
        <div class="month-selection">
            ...
            <div class="previous-month">
                <a href="{{ route('admin.attendance.staff.list', ['id' => $user->id,'month' => $prevMonth]) }}">
        ...
        <table class="staff-attendance-list__table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    {{-- モデルのメソッドを呼び出す --}}
                    {{--休憩合計時間--}}
                    <td>{{ $attendance->id ? $attendance->getFormattedTotalBreakTime() : '00:00' }}</td>
                    {{--（勤務時間）合計--}}
                    <td>{{ $attendance->id ? $attendance->getFormattedTotalWorkingTime() : '00:00' }}</td>
                    
                    <td>
                        <a href="{{ route('admin.attendance.show', ['id' => $attendance->id ?? $attendance->date, 'user_id' => $user->id]) }}" class="detail-button">詳細</a>
                    </td>
                </tr>
                @endforeach
            </tbody>
